...
var (
	...
	Configor = "configor"
	Viper    = "viper"
	remoteRepo = "git.basebit.com/mixcli"
	srcDir = "./template"
)
...
func (t *NewCommand) Main() {
	...
	switch strings.ToLower(promp("Select project type", []string{"API", "gRPC", "TASK", "CRON"})) {
	case Task:
		selectType = Task
		break
	case API:
...
func (t *NewCommand) NewProject(name, selectType string) {
	...
	//srcDir := fmt.Sprintf("%s/pkg/mod/%s@%s/template/%s", goPath,remoteRepo,ver, selectType)
	srcDir = srcDir + fmt.Sprintf("/%s",selectType)
	//if _, err := os.Stat(srcDir); err != nil {
	//	cmd := exec.Command("go", installCmd, fmt.Sprintf("%s@%s", remoteRepo, ver))
	//	fmt.Printf("Skeleton local not found, exec 'go %s %s@%s'\n", installCmd, remoteRepo, ver)
	//	total := 1000
	//	current := int64(0)
	...
	//if err := logic.ReplaceAll(dest, fmt.Sprintf("github.com/mix-go/%s-skeleton",remoteRepo, selectType), name); err != nil {
	//	panic(errors.New("Replace failed"))
	//}
